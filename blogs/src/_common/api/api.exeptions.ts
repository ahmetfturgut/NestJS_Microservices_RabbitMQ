...
export class ApiException extends HttpException {
    ...
    static buildFromApiError(apiError: ApiError, data?: Map<string, string>) {
        if (data) {
            ...
            return new RpcException(new ApiException(apiError.getErrorCode(), message, data));

        } else {
            return new RpcException(new ApiException(apiError.getErrorCode(), apiError.getMessage()));
        }
    }
}
